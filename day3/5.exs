...
defmodule CrossedWires do
  def find_first_intersection(wire1, wire2) do
    ...
    IO.puts("wire 1 path created with length #{Enum.count(wire1_path)}")
    ...
    IO.puts("wire 2 path created with length #{Enum.count(wire2_path)}")

    find_intersection_points(wire1_path, wire2_path)
    |> find_manhatten_distances()
    |> Enum.at(0)
  end
  ...
  defp find_intersection_points(wire1_path, wire2_path) do
    points = wire1_path
    |> Enum.filter(fn position_1 ->
      Enum.find(wire2_path, fn position_2 -> position_2 == position_1 end)
    end)
    
    IO.puts("Found intersection points with length #{Enum.count(points)}")
    points
  end

  defp find_manhatten_distances(coordinates) do
    dists = coordinates
    |> Enum.map(fn {x, y} -> abs(x) + abs(y) end)
    |> Enum.sort()

    IO.puts("Found manhatten distances with length #{Enum.count(dists)}")
    dists
  end
...
# wire 1 path created with length 146736 -> fast 
# wire 2 path created with length 154555 -> fast
# Found intersection points with length 44 -> SLOW. Because needs to traverse both arrays with nested loops
# Found manhatten distances with length 44 -> fast
